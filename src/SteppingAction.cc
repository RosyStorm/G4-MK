...
/// \brief SteppingAction 类的实现：步级用户动作
///
...
// 变电荷态时会创建新粒子定义(alpha->alpha+->alpha++->helium)，用 G4Alpha::Definition()
// 只匹配第一级，漏掉后续链上粒子。这里改用粒子名判定覆盖完整 α 链。
namespace {
/// 判断给定粒子是否属于 α 链(He2+/He+/He0)。
/// @param pd 待判定的粒子定义
/// @return 属于 α 链返回 true，否则 false
...
{
  /// 每一步的动作：累加全局能量沉积、执行出核/出细胞粒子杀死、
  /// 补加出射边界步的核内能量沉积、累计 α 链投影射程。
  /// @param step 当前步对象

  // —— 任务6.2：累加全局能量沉积（须在任何 kill 之前）——
  // 全部体积、全部粒子，用于能量平衡校验
  ...
  if (detector && detector->GetKillOutsideCell()) {
    // kill 半径：默认 R_n(出核即杀，更快且核打分无偏)，可切回 R_cell
    G4double killRadius =
      detector->GetKillAtNucleus() ? detector->GetNucleusRadius()
                                   : detector->GetCellRadius();
  ...
  // 入射步 (pre 在核外 post 在核内) 由 SD 的 hit-sum 自然覆盖；核内 step 同理。
  {
    ...
    const auto* preVolume  = pre->GetTouchableHandle()->GetVolume();
    const auto* postVolume = post->GetTouchableHandle()->GetVolume();
    if (preVolume && postVolume &&
        preVolume->GetName() == "Nucleus" && postVolume->GetName() != "Nucleus") {
      if (fEventAction)
